...
func (a *DefaultApiService) VerifyCheck(ctx _context.Context, format string, apiSecret string, requestId string, code string, localVarOptionals *VerifyCheckOpts) (CheckResponse, *_nethttp.Response, error) {
	...
	localVarQueryParams.Add("request_id", parameterToString(requestId, ""))
...
Control the progress of your Verify requests. To cancel an existing Verify request, or to trigger the next verification event:   1. Send a Verify control request with the appropriate command (&#x60;cmd&#x60;) for what you want to achieve.  2. Check the &#x60;status&#x60; in the response.
 * @param ctx _context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param format The response format.
 * @param apiSecret You can find your API secret in your Nexmo account [developer dashboard](https://dashboard.nexmo.com)
...
func (a *DefaultApiService) VerifyRequest(ctx _context.Context, format string, apiSecret string, number string, brand string, localVarOptionals *VerifyRequestOpts) (RequestResponse, *_nethttp.Response, error) {
	...
	localVarQueryParams.Add("brand", parameterToString(brand, ""))
	if localVarOptionals != nil && localVarOptionals.SenderId.IsSet() {
		localVarQueryParams.Add("sender_id", parameterToString(localVarOptionals.SenderId.Value(), ""))
	}
	if localVarOptionals != nil && localVarOptionals.CodeLength.IsSet() {
		localVarQueryParams.Add("code_length", parameterToString(localVarOptionals.CodeLength.Value(), ""))
	}
	if localVarOptionals != nil && localVarOptionals.Lg.IsSet() {
		localVarQueryParams.Add("lg", parameterToString(localVarOptionals.Lg.Value(), ""))
	}
	if localVarOptionals != nil && localVarOptionals.PinExpiry.IsSet() {
		localVarQueryParams.Add("pin_expiry", parameterToString(localVarOptionals.PinExpiry.Value(), ""))
	}
	if localVarOptionals != nil && localVarOptionals.NextEventWait.IsSet() {
		localVarQueryParams.Add("next_event_wait", parameterToString(localVarOptionals.NextEventWait.Value(), ""))
	}
	if localVarOptionals != nil && localVarOptionals.WorkflowId.IsSet() {
		localVarQueryParams.Add("workflow_id", parameterToString(localVarOptionals.WorkflowId.Value(), ""))
	}
	// to determine the Content-Type header
	localVarHTTPContentTypes := []string{}
